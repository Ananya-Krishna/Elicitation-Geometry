...
For each model_family in {llama2, falcon, mistral}, this script:

  1. Loads SAE encodings from new_sae_outputs:

        new_sae_outputs/
          llama2/run_YYYYMMDD_HHMMSS/
            llama2_base_sae_encoded.npy
            llama2_aligned_sae_encoded.npy
          falcon/run_.../
            falcon_base_sae_encoded.npy
            falcon_aligned_sae_encoded.npy
          mistral/run_.../
            mistral_base_sae_encoded.npy
            mistral_aligned_sae_encoded.npy

     The latest run_* directory (by name) is used for each model_family.

  2. Loads Neural-ODE actions:
     - {base_path}/{ode_subdir}/{model_family}/{model_family}_actions.npy
...
def load_encodings(sae_root: Path, model_family: str):
    """
    Load SAE encodings for a given model_family from new_sae_outputs/.
    ...
    Expects structure:
      sae_root/
        ...
        falcon/run_YYYY.../
        mistral/run_YYYY.../
    and filenames inside the chosen run dir:
      {model_family}_base_sae_encoded.npy
      {model_family}_aligned_sae_encoded.npy

    Picks the latest run_* directory by name (your run dirs are timestamped).
    """
    model_root = sae_root / model_family
    if not model_root.exists():
        raise FileNotFoundError(f"SAE root not found for {model_family}: {model_root}")
    ...
    if not run_dirs:
        raise FileNotFoundError(f"No run_* dirs found for {model_family} in {model_root}")

    use_dir = run_dirs[-1]  # latest by name
    base_path = use_dir / f"{model_family}_base_sae_encoded.npy"
    aligned_path = use_dir / f"{model_family}_aligned_sae_encoded.npy"

    if not base_path.exists() or not aligned_path.exists():
        raise FileNotFoundError(
            f"Missing encodings for {model_family}: {base_path}, {aligned_path}"
        )
    ...
    if base.shape != aligned.shape:
        raise ValueError(
            f"Shape mismatch for {model_family}: base {base.shape}, aligned {aligned.shape}"
        )

    print(f"[{model_family}] Loaded encodings from {use_dir}: {base.shape}")
...
def load_toxicity_scores(data_dir: Path, n_samples: int):
    ...
    # Some debug histograms/ranges are useful to sanity-check
...
def plot_action_vs_toxicity(model_family, actions, toxicity, fig_dir: Path):
    # --- Start 3 Sigma Filtering ---
    # Ensure numpy arrays for masking
    actions = np.array(actions)
    toxicity = np.array(toxicity)
    
    # Calculate stats and create mask
    mean = np.mean(actions)
    std = np.std(actions)
    # Keep only data within 3 standard deviations
    mask = np.abs(actions - mean) <= (3 * std)
    
    # Apply filter to both arrays to keep them aligned
    actions = actions[mask]
    toxicity = toxicity[mask]
    print(f"    [filter] Removed {np.sum(~mask)} outliers (>3 sigma)")
    # --- End 3 Sigma Filtering ---
...
def plot_action_by_tox_bin(model_family, actions, toxicity, fig_dir: Path):
    ...
    for i in range(len(bins) - 1):
        ...
        mask_bin = (toxicity >= lo) & (toxicity < hi)
        if mask_bin.sum() > 0:
            data.append(actions[mask_bin])
        else:
            data.append(np.array([]))

    plt.figure(figsize=(6, 4))
    
    # Optional: You can also add showfliers=False here if you 
    # want to hide the remaining statistical outliers in the boxplot visualization.
...
def analyze_model(args_tuple):
    ...
    Wrapper for multiprocessing:
      (model_family, base_path, sae_subdir, ode_subdir, data_dir, output_dir)
    """
...
def main():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="new_approach/analysis_output_final",
